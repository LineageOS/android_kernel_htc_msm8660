...
static int mipi_novatek_lcd_on(struct platform_device *pdev)
{
	...
	if (mipi->mode == DSI_VIDEO_MODE) {
		...
		if (1) // mod this to 0 for old wvga
		{
			mipi_dsi_cmds_tx(&novatek_tx_buf, novatek_cmd_on_cmds,
					ARRAY_SIZE(novatek_cmd_on_cmds));
		} else
		{
			mipi_dsi_cmds_tx(&novatek_tx_buf, novatek_wvga_c3_cmd_on_cmds, //novatek_cmd_on_cmds,
					ARRAY_SIZE(novatek_wvga_c3_cmd_on_cmds));//novatek_cmd_on_cmds));
		}
...
static int __devinit mipi_novatek_lcd_probe(struct platform_device *pdev)
{
	...
	pr_err("NOVATEK sec probe success\n");
	return 0;
}
