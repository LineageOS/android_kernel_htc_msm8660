...
// TODO rate? --> hw_init -> rate not set?
		mdp_clk_ctrl(1);
		...
		mdp_hw_init();
#endif
		MSM_FB_DEBUG("mdp probe mdp-hw-init done\n");

#ifdef CONFIG_FB_MSM_OVERLAY
		mdp_hw_cursor_init();
		MSM_FB_DEBUG("mdp probe mdp-hw-cursor init done\n");
#endif
		mdp_clk_ctrl(0);

		mdp_resource_initialized = 1;
		MSM_FB_DEBUG("mdp probe res init done\n");
		return 0;
	}
	// TODO no pdev->id > 0 ??? why?
	...
	if (!mdp_bus_scale_handle && mdp_pdata &&
		mdp_pdata->mdp_bus_scale_table) {
			printk(KERN_ERR "%s [[]] register bus scale client\n",
				__func__);
