...
static int mipi_dsi_probe(struct platform_device *pdev)
{
	...
	pr_err("mipi_dsi: pdev %d resnum %d \n", pdev->id, pdev->num_resources);
	...
	if ((pdev->id == 1) && (pdev->num_resources >= 0)) {
		...
		if (mipi_dsi_pdata) {
			pr_err("mipi_dsi: pdev %d resnum %d : mipi_dsi_pdata is not null, vsync \n", pdev->id, pdev->num_resources);
			vsync_gpio = mipi_dsi_pdata->vsync_gpio;
			pr_err("%s: vsync_gpio=%d\n", __func__, vsync_gpio);
			...
			if (mdp_rev == MDP_REV_303 &&
				...
				if (mipi_dsi_pdata->dsi_client_reset())
					...
					pr_err("%s: DSI Client Reset success\n",
						__func__);
			}
		} else
		{
			pr_err("mipi_dsi: pdev %d resnum %d : mipi_dsi_pdata is NULL, no vsync \n", pdev->id, pdev->num_resources);
		}
	...
	pr_err("%s:bpp=>%d, lanes=>%d, dsi_pclk_rate=>%d", __func__, bpp, lanes, dsi_pclk_rate);

//	if (1) return -300;

// TODO: ERROR IS BELOW, PHONE POWERS OFF
